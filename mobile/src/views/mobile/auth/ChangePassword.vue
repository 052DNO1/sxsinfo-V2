...
export default {
  ...
  setup() {
    ...
    const { post: changePasswordApi, get: fetchPasswordInfoApi } = useApi('/userinfo/change_password/', { immediate: false })
    ...
    const fetchFirstLoginInfo = async () => {
      try {
        const response = await fetchPasswordInfoApi()
        if (response && response.success !== false) {
          const isFirst = response.first_login === true || response.first_login === 'true' || response.first_login === 1
          if (isFirst) {
            isFirstLogin.value = true
            if (response.message) firstLoginMessage.value = response.message
          }
        }
      } catch (err) {
    ...
    const saveSecurityQuestion = async () => {
      ...
      try {
        const { post: saveApi } = useApi('/userinfo/security_question/', { immediate: false })
