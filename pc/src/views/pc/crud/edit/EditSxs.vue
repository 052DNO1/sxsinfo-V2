    ...
    <template #rightcontent>
      <div class="page-container">
        <div class="listheader custom-header">
          <span class="header-text"><el-icon><HomeFilled /></el-icon> {{ header || '编辑实训室' }}</span>
          ...
          <div class="unified-panel">
            ...
            <div class="panel-main">
              <div class="main-header">
                <h3><el-icon><EditPen /></el-icon> {{ header || '编辑实训室' }}</h3>
              </div>
                  ...
                  <el-row :gutter="24">
                    <template v-for="field in formFields" :key="field.name">
                      ...
                      <el-col :span="12" v-else-if="field.type === 'checkbox'">
                        <el-form-item :label="field.label" :prop="field.name" class="custom-form-item">
...
const loadData = async () => {
  ...
  try {
    ...
    const [labResponse, adminResponse] = await Promise.all([
      apiComposable.get({}, { url: `/laboratories/${labId}/` }),
      apiComposable.get({}, { url: '/laboratories/admin_options/' })
    ])
    
    const data = labResponse.data || labResponse
    const adminData = adminResponse.data || adminResponse
    
    header.value = data.header || '编辑实训室'
    
    const rawAdmins = adminData.admins || adminData.admin_choices || []
    const adminChoices = rawAdmins.map(a => ({
        value: a.id,
        label: a.id === 0 ? '未分配' : `${a.nickname || ''} (${a.username})`
    }))
