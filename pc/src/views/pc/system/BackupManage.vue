              ...
              <div class="main-content" :class="{ 'list-mode': showBackupList }">
                <template v-if="!showBackupList">
                  <div class="operation-section">
                    ...
                    <p class="section-desc">导出系统所有数据为JSON文件，可用于数据迁移或灾难恢复</p>
                    ...
                    <div class="backup-content-box">
                      <div class="backup-item" v-for="item in backupItems" :key="item.key">
                        ...
                        <span class="item-count">{{ item.count || 0 }} 条</span>
                      </div>
                    </div>
                  ...
                  <div class="operation-section">
                    ...
                    >
                      <div class="upload-content">
                        ...
                        <div class="upload-hint">仅支持上传 .json 格式的备份文件</div>
                      </div>
                    </el-upload>
                    ...
                    <div v-if="backupInfo" class="backup-info-card">
                      ...
                      <div class="info-grid">
                        ...
                        <div class="info-item">
                          ...
                          <span class="value">{{ backupInfo.created_at }}</span>
                        </div>
                        <div class="info-item">
                ...
                <template v-else>
                  <div class="operation-section list-section">
                    ...
                    <el-table :data="backupList" v-loading="listLoading" stripe class="backup-table" style="width: 100%">
                      ...
                      <el-table-column prop="backup_type_display" label="类型" width="100" align="center">
...
export default {
  ...
  setup() {
    ...
    const backupItems = ref([
      { key: 'users', name: '用户数据', icon: 'User', count: 0 },
      { key: 'departments', name: '部门数据', icon: 'OfficeBuilding', count: 0 },
      { key: 'terms', name: '学期数据', icon: 'Calendar', count: 0 },
      { key: 'labs', name: '实训室数据', icon: 'Monitor', count: 0 },
      { key: 'lab_classes', name: '课表数据', icon: 'Reading', count: 0 },
      { key: 'lab_records', name: '使用记录', icon: 'Document', count: 0 },
      { key: 'lab_maintains', name: '维护记录', icon: 'Tools', count: 0 },
      { key: 'equipment', name: '设备数据', icon: 'Monitor', count: 0 },
    ])
    ...
    const loadAutoBackupConfig = async () => {
      try {
        ...
        if (response) {
          autoBackupEnabled.value = response.enabled || false
          autoBackupPeriod.value = response.period || 'daily'
      ...
      try {
        ...
        if (response && response.stats) {
          backupItems.value = backupItems.value.map(item => ({
            ...item,
            count: response.stats[item.key] || 0
          }))
        }
      } catch (error) {
      }
    ...
    const handleBackup = async () => {
      try {
        ...
        const response = await get({}, { url: '/backups/export/', responseType: 'blob' })
        ...
        if (!(blobData instanceof Blob)) {
          blobData = new Blob([blobData], { type: 'application/json' })
        }
