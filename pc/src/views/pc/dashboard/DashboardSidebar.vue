    ...
    <el-card class="menu-card" shadow="hover" :body-style="{ padding: '0' }">
      ...
      >
        ...
        <el-sub-menu index="main-work">
          ...
          <!-- 系统管理员（is_superuser=True）：只显示系统管理控制台 -->
          <template v-if="user?.is_superuser">
            <el-menu-item index="system-admin" @click="handleNavigate('/system-admin')">
...
import {
  ...
  DataLine, Expand, Fold, Grid, Plus, Check, Download, Tickets, Monitor
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const allMenuItems = computed(() => {
      ...
      if (u.is_systemadmin) {
        items.push(
          { id: 'system-admin', path: '/system-admin', icon: 'Monitor', label: '系统管理控制台' }
        )
      }
