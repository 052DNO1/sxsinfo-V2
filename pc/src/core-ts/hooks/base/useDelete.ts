...
import type { UseDeleteOptions } from '../../types'

interface UseDeleteReturn {
  handleDelete: (item: any) => Promise<void>
  handleBatchDelete: (selection: any[]) => Promise<void>
}
...
export function useDelete(options: UseDeleteOptions = {}): UseDeleteReturn {
  ...
  const handleDelete = async (item: any): Promise<void> => {
    ...
    try {
      ...
      if (response && (response as any).success !== false) {
        ...
        await showError((response as any)?.message || '删除失败')
      }
    } catch (err) {
      await showError(err)
    }
  ...
  const handleBatchDelete = async (selection: any[]): Promise<void> => {
    ...
    if (handleItemDelete && typeof handleItemDelete === 'function') {
      const successItems: (number | string)[] = []
      const failedItems: { id: number | string; reason: string }[] = []
      ...
      try {
        ...
        if (response && (response as any).success) {
          ...
          if (deletedCount > 0 && failedCount > 0) {
            ...
            await showSuccess((response as any).message || `成功删除 ${deletedCount} 项`)
          } else if (failedCount > 0) {
            let warnMsg = (response as any).message || '删除失败'
            ...
            await showSuccess((response as any).message || '操作完成')
          }
          ...
          const failedItems = extractFailedItems(response)
          let errorMsg = (response as any)?.message || '批量删除失败'
