<template>
  <MobileLayout>
    <div class="mobile-page">
      <van-nav-bar title="学期归档" left-arrow @click-left="goBack">
        <template #right><van-icon name="home-o" size="20" color="#4F6EF7" @click="goHome" /></template>
      </van-nav-bar>

      <div class="page-content">
        <div class="archive-hero animate-fade-in-up" v-if="currentTerm">
          <div class="current-term-card" :class="{ archived: currentTerm.is_archived }">
            <div class="card-badge" :class="currentTerm.is_archived ? 'archived' : 'current'">
        ...
        <div class="empty-current animate-fade-in-up" v-else>
          <van-icon name="warning-o" size="24" color="#FF9500" />
          <span>未设置当前学期</span>
        </div>

        <div v-if="currentTermStats" class="stats-section animate-fade-in-up animate-delay-1">
          <div class="section-label"><span>📊</span> 学期数据统计</div>
          <div class="stats-grid">
            <div class="stat-card">
              <van-icon name="cluster-o" size="20" color="#07C160" />
              <div class="stat-info">
            ...
            <div class="stat-card">
              <van-icon name="desktop-o" size="20" color="#5AC8FA" />
              <div class="stat-info">
            ...
            <div class="stat-card">
              <van-icon name="friends-o" size="20" color="#4F6EF7" />
              <div class="stat-info">
            ...
            <div class="stat-card">
              <van-icon name="description" size="20" color="#9B59B6" />
              <div class="stat-info">
            ...
            <div class="stat-card">
              <van-icon name="calendar-o" size="20" color="#FF9500" />
              <div class="stat-info">
            ...
            <div class="stat-card">
              <van-icon name="setting-o" size="20" color="#FF3B30" />
              <div class="stat-info">
        ...
        <div class="info-section animate-fade-in-up animate-delay-2" v-if="termList.length > 0">
          <div class="section-label"><span>📦</span> 历史学期</div>
          <div class="term-list">
            <div v-for="item in termList" :key="item.id" class="term-card" :class="{ archived: item.is_archived }">
              <div class="term-name">{{ item.name || item.termname }}</div>
              <div class="term-date">{{ item.start_date }} ~ {{ item.end_date }}</div>
              <span v-if="item.is_archived" class="status-tag success">已归档</span>
            </div>
          </div>
        </div>
        ...
        <template v-if="!currentTerm?.is_archived && canArchive">
          <div class="archive-section animate-fade-in-up animate-delay-3">
            <div class="section-label"><span>⚙️</span> 选择归档数据类型</div>
            <p class="section-desc">归档后数据会从业务表物理删除，仅保留在归档表中</p>

            <div class="options-list">
              <div class="option-card" :class="{ selected: archiveTypes.lab_info }" @click="toggleArchiveType('lab_info')">
                <div class="option-checkbox">
                  <van-checkbox :model-value="archiveTypes.lab_info" shape="square" />
                </div>
              ...
              <div class="option-card" :class="{ selected: archiveTypes.device_info }" @click="toggleArchiveType('device_info')">
                <div class="option-checkbox">
                  <van-checkbox :model-value="archiveTypes.device_info" shape="square" />
                </div>
              ...
              <div class="option-card" :class="{ selected: archiveTypes.user_info }" @click="toggleArchiveType('user_info')">
                <div class="option-checkbox">
                  <van-checkbox :model-value="archiveTypes.user_info" shape="square" />
                </div>
            ...
            <div class="dept-section" v-if="departments.length > 0">
              <div class="section-label"><span>🏢</span> 按部门筛选（可选）</div>
              <van-field
          ...
          <div class="tips-section animate-fade-in-up animate-delay-4">
            <div class="tips-header"><van-icon name="warning-o" size="16" /> 重要提示</div>
            <ul class="tips-list">
        ...
        <div v-else-if="currentTerm?.is_archived" class="archived-notice animate-fade-in-up">
          <van-icon name="lock" size="48" color="#909399" />
          <h4>当前学期已归档</h4>
          <p>归档后的数据只能查看，不能修改。如需修改请联系超级管理员。</p>
          <van-button type="primary" size="small" @click="router.push('/archived-terms')">查看归档记录</van-button>
        </div>

        <div class="quick-actions animate-fade-in-up animate-delay-2" v-if="canArchive && !currentTerm?.is_archived">
          <div class="action-card primary" @click="router.push('/archived-terms')">
            <van-icon name="search" size="20" /><span>查看归档记录</span>
          </div>
        </div>
      </div>
    </div>
