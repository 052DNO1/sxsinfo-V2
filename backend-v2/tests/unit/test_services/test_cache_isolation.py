...
class TestCacheUserIsolation:
    ...
    def test_different_users_get_different_data(self):
        ...
        dept_lab_ids = set(lab['id'] for lab in dept_result['list'])
        lab_lab_ids = set(lab['id'] for lab in lab_result['list'])
        assert dept_lab_ids != lab_lab_ids, \
    ...
    def test_cache_isolation_rapid_refresh(self):
        ...
        dept_ids = set(r['lab_ids'][0] for r in results_dept if r['lab_ids'])
        lab_ids = set(r['lab_ids'][0] for r in results_lab if r['lab_ids'])
        assert results_dept[0]['lab_ids'] != results_lab[0]['lab_ids'], \
            "分院管理员和实训室管理员应该看到不同的实训室范围"
    ...
    def test_cache_isolation_alternating_requests(self):
        ...
        for i in range(5):
            ...
            assert self.lab1.id in dept_ids, \
                f"第{i+1}轮: 分院管理员应该看到计算机实训室1"

            assert self.lab1.id in lab_ids, \
                f"第{i+1}轮: 实训室管理员应该看到自己管理的实训室"
