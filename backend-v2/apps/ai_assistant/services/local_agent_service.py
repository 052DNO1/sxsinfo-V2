...
class LocalAgentService:
    ...
    def process_request(self, user, question: str, session_id: str = None) -> dict:
        context = {}
        if session_id:
            context = cache.get(f'local_agent_session_{session_id}', {})
        ...
        if missing_fields:
            ...
            cache.set(f'local_agent_session_{new_session_id}', context, timeout=3600)
    ...
    def execute_operation(self, user, auth_token: str) -> dict:
        context = cache.get(f'local_agent_session_{auth_token}')
        if not context:
            return {
                'success': False,
                'error': '操作已过期，请重新发起请求'
        ...
        cache.delete(f'local_agent_session_{auth_token}')
    ...
    def _query_laboratories(self, user, params: dict) -> dict:
        ...
        total_capacity = sum(lab.capacity for lab in queryset)
